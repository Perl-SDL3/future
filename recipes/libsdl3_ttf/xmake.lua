        ...
        if freetype then
            ...
            if fetchinfo then
                ...
                if not freetype:config("shared") then
                    ...
                    if #libfiles == 0 and freetype:is_system() and package:is_plat("macosx") then
                        -- A statically-linked system (Homebrew) freetype drags in its own private
                        -- link-time libraries (bzip2, zlib, libpng, brotli), but system packages
                        -- expose no librarydeps for xrepo to append, so linking the shared library
                        -- fails with undefined symbols such as _BZ2_bzDecompress. Recover those
                        -- flags from freetype's pkg-config Libs.private instead.
                        local pc
                        for _, prefix in ipairs({ os.getenv("HOMEBREW_PREFIX") or "", "/opt/homebrew", "/usr/local" }) do
                            local pcpath = path.join(prefix, "opt", "freetype", "lib", "pkgconfig", "freetype2.pc")
                            if os.isfile(pcpath) then
                                pc = io.readfile(pcpath)
                                break
                            end
                        end
                        if pc then
                            local section, started = {}, false
                            for line in pc:gmatch("[^\r\n]+") do
                                local key, value = line:match("^(%a[%w%._-]*):%s*(.*)$")
                                if key then
                                    if started then
                                        break
                                    end
                                    value = value or ""
                                    if key == "Libs.private" then
                                        table.insert(section, value)
                                        started = true
                                    end
                                elseif started and line:match("^%s+") then
                                    table.insert(section, line:gsub("^%s+", ""))
                                end
                            end
                            local private = table.concat(section, " ")
                            private = private:gsub("%s+", " "):gsub("^%s+", ""):gsub("%s+$", "")
                            if #private > 0 then
                                table.insert(libfiles, private)
                            end
                        end
                    end
                    if #libfiles > 0 then
